...
def _force_place(bi: int,
    ...
    """
    Fallback placement: place block bi using an empty-bay entry window
    (guaranteed crane-feasible, no Shapely calls -- see reasoning below),
    choosing among ALL bays the one that gives the EARLIEST entry time
    (myalgorithm4 change vs baseline_greedy.py, which stopped at the first
    fitting bay in preference order).

    Why this is a safe, cheap "safety net" refinement:
      _empty_bay_entry is pure integer/interval arithmetic (no Shapely), so
      computing it for every fitting bay costs essentially nothing extra.
      When this fallback fires (time-guard force-placement, or a genuine
      repeat-violator cycle-break), tardiness is already the overriding
      concern -- picking whichever bay is free soonest directly reduces the
      Z1 damage a force-placed block does, at negligible cost.  Preference
      is used only as a tie-break among bays with the same earliest entry.

      An earlier attempt to make this fallback "smarter" by re-running the
      real _find_earliest_slot search here was reverted (see git history/
      earlier docstring) because it broke the repeat-violator cycle-breaking
      guarantee and caused a genuine INFEASIBLE regression.  This change is
      much more conservative: it keeps the exact same guaranteed-safe
      empty-bay mechanism, only choosing WHICH bay via a cheap comparison
      instead of first-match, so it cannot reintroduce that risk.
        ...
        if (timelimit is not None and t_start is not None
            ...
            for bid in remaining:
                bay_id, cx, cy, oi, entry, exit_t = _force_place(
                    bid, blocks_data, bays, bay_schedule,
